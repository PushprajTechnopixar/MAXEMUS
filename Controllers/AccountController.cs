...
namespace MaxemusAPI.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        #region DealerRegistration
        /// <summary>
        ///  Registration for Dealer.
        /// </summary>
        /// <returns></returns>
        [HttpPost]
        [Route("DealerRegistration")]
        [AllowAnonymous]
        public async Task<IActionResult> DealerRegistration([FromBody] DealerDetailDTO model)
        {
            bool ifUserNameUnique = _userRepo.IsUniqueUser(model.email, model.phoneNumber);
            if (!ifUserNameUnique)
            {
            ...
            var dealer = await _userRepo.DealerRegistration(model);
            if (dealer == null)
            {
                _response.StatusCode = HttpStatusCode.OK;
                _response.IsSuccess = false;
            ...
            //if (dealer.role == Role.Dealer.ToString())
            //{
            //    // Send email here
        ...
        /// <returns></returns>
        [HttpPost]
        [Route("DistributorRegistration")]
        [AllowAnonymous]
        public async Task<IActionResult> DistributorRegistration([FromBody] DistributorDTO model)
        {
            bool ifUserNameUnique = _userRepo.IsUniqueUser(model.Email, model.PhoneNumber);
            if (!ifUserNameUnique)
            {
            ...
            var distributor = await _userRepo.DistributorRegistration(model);
            if (distributor == null)
            {
                _response.StatusCode = HttpStatusCode.OK;
                _response.IsSuccess = false;
